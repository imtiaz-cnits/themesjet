...
export default function ProductsBrowser({
  ...
  const currentSortLabel =
    sortOptions.find((o) => o.value === currentSortValue)?.label || "Newest";
  ...
  const handleManualInputSubmit = () => {
    updateQuery({
      ...
      maxPrice:
        priceRange[1] < MAX_PRICE_LIMIT ? priceRange[1].toString() : null,
    });
  };
  ...
  const filterUI = (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        searchParams.toString().length > 7 ? (
          <button
      ...
      <div className="bg-card border border-border rounded-xl p-5 shadow-sm">
        <h3 className="font-bold text-foreground text-xs mb-4 uppercase tracking-wider">
          Price Range
        </h3>
          ...
          <div className="flex gap-3 items-center mt-4">
            <div className="relative w-1/2">
              ...
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && handleManualInputSubmit()
                }
            ...
            <div className="relative w-1/2">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-primary text-xs font-bold">
                $
              </span>
              <input
                type="number"
              ...
              >
                <div className="relative flex items-center">
                  <input
                    ...
                    checked={isActive}
                    onChange={() =>
                      updateFilter("framework", isActive ? "" : fw)
                    }
                ...
                <span
                  className={`text-sm font-medium transition-colors ${isActive ? "text-primary" : "text-muted-foreground group-hover:text-foreground"}`}
                >
                  {fw}
                </span>
              </label>
          ...
          {[5, 4].map((stars) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-muted-foreground text-xs font-bold group-hover:text-foreground">
                  & Up
                </span>
              </label>
            );
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-start">
        ...
        <main className="flex-1 w-full">
          ...
          <div className="flex flex-nowrap justify-between items-center mb-8 gap-3 bg-card p-2 rounded-xl border border-border shadow-sm">
            ...
            <div className="flex gap-1 items-center gap-2 ml-auto">
              <div className="flex items-center bg-background border border-border rounded-lg p-1">
                <button
                  onClick={() => setViewMode("grid")}
                  className={`p-1.5 rounded cursor-pointer transition-colors ${viewMode === "grid" ? "bg-muted text-primary" : "text-muted-foreground hover:bg-muted"}`}
                >
                  <LayoutGrid size={18} />
                </button>
                <button
                  onClick={() => setViewMode("list")}
                  className={`p-1.5 rounded cursor-pointer transition-colors ${viewMode === "list" ? "bg-muted text-primary" : "text-muted-foreground hover:bg-muted"}`}
                >
                  <List size={18} />
                </button>
              </div>

              {/* --- CUSTOM NICE SELECT DROPDOWN --- */}
              <div className="relative" ref={sortRef}>
                {/* Trigger Button */}
                <button
                  onClick={() => setIsSortOpen(!isSortOpen)}
                  className={`flex items-center justify-between min-w-[170px] px-4 py-2.5 bg-background border rounded-lg text-sm font-bold transition-all duration-200 outline-none
                                    ${isSortOpen ? "border-primary ring-1 ring-primary text-foreground" : "border-border text-foreground hover:border-primary/50"}`}
                >
                ...
                <AnimatePresence>
                  {isSortOpen && (
                    <motion.ul
                      ...
                      className="absolute right-0 top-full mt-2 w-[180px] bg-card border border-border rounded-xl shadow-xl shadow-black/5 z-50 overflow-hidden py-1"
                    >
              ...
              >
                {initialProducts.map((product) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`${viewMode === "grid" ? "h-48" : "h-48 w-full md:w-64 md:h-auto"} relative overflow-hidden bg-muted flex-shrink-0`}
                      >
                        <Image
                        ...
                        {product.badge && (
                          <div className="absolute top-3 left-3 z-10">
                            <span
                              className={`px-2.5 py-1 ${product.badgeColor} backdrop-blur-md text-white text-[10px] font-bold rounded uppercase tracking-wider shadow-lg`}
                            >
                              {product.badge}
                            </span>
                          </div>
              ...
              {totalPages > 1 && (
                <div className="flex justify-center gap-2 mt-8">
                  <button
                    ...
                    className="w-10 h-10 cursor-pointer flex items-center justify-center rounded-lg border border-border text-muted-foreground hover:border-primary hover:text-primary transition-colors font-bold text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    &lt;
                  </button>
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                    (page) => (
                      <button
                        ...
                        className={`w-10 h-10 cursor-pointer flex items-center justify-center rounded-lg font-bold text-sm transition-colors ${currentPage === page ? "bg-primary text-primary-foreground shadow-lg shadow-primary/20" : "border border-border text-muted-foreground hover:border-primary hover:text-primary"}`}
                      >
                        {page}
                      </button>
                    ),
