...
import { Download, ArrowUpRight, DollarSign, Calendar } from "lucide-react";
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { label: "Available for Payout", amount: "$1,204.50", sub: "Minimum payout: $50.00" },
                    { label: "Total Earnings", amount: "$14,204.00", sub: "+12.5% this month" },
                    { label: "Pending Clearance", amount: "$340.00", sub: "Clears in ~3 days" },
                ].map((card, i) => (
                    <motion.div
                        key={i}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-card border border-border rounded-2xl p-6 shadow-sm flex flex-col">
                    ...
                    {/* Mock Chart UI */}
                    <div className="flex-1 flex items-end justify-between gap-2 h-64 px-4 pb-4">
                        {[40, 65, 45, 80, 55, 70, 90, 60, 75, 50, 85, 95].map((h, i) => (
                            <div key={i} className="w-full bg-primary/10 rounded-t-lg relative group">
                                <motion.div
                                    ...
                                    className="absolute bottom-0 w-full bg-primary/80 rounded-t-lg group-hover:bg-primary transition-colors"
                                />
                            </div>
                        ))}
                    </div>
