...
import { Github, Check, ArrowRight, ArrowLeft, User, Mail, Lock, AlertCircle, Loader2 } from "lucide-react";
import { register, socialLogin } from "@/actions/auth"; // Ensure 'register' is exported from actions/auth.ts
...
export default function RegisterPage() {
    ...
    const getStrengthColor = (index: number) => {
        if (index >= passwordStrength) return "bg-input/20"; // Empty slot
    ...
    return (
        <div className="min-h-screen flex bg-background text-foreground font-body transition-colors duration-300 relative overflow-hidden">
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-center relative z-10 p-6 lg:p-12 xl:p-24 overflow-y-auto">
                ...
                <div className="max-w-md w-full mx-auto mt-16 lg:mt-0">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                        <button type="button" onClick={() => socialLogin("google")} className="flex cursor-pointer items-center justify-center gap-2 py-2.5 bg-card border border-border rounded-lg hover:bg-secondary transition-all text-sm font-bold text-foreground">
                        ...
                        <button type="button" onClick={() => socialLogin("github")} className="flex cursor-pointer items-center justify-center gap-2 py-2.5 bg-card border border-border rounded-lg hover:bg-secondary transition-all text-sm font-bold text-foreground">
                    ...
                    <form className="space-y-5" onSubmit={handleSignup}>
                        ...
                        {success && (
                            <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/20 flex items-center gap-3 text-green-500 text-sm font-bold">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <div className="relative">
                                    <input name="firstName" type="text" required className="w-full bg-input/10 border border-input rounded-lg pl-10 pr-4 py-3 text-foreground outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all placeholder:text-muted-foreground/50" placeholder="John" />
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <input name="lastName" type="text" required className="w-full bg-input/10 border border-input rounded-lg pl-10 pr-4 py-3 text-foreground outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all placeholder:text-muted-foreground/50" placeholder="Doe" />
                                    <User className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground w-4 h-4" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full bg-input/10 border border-input rounded-lg pl-10 pr-4 py-3 text-foreground outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
                                />
                            ...
                            <p className="text-[10px] text-muted-foreground mt-1 text-right font-medium">
                                Strength: <span className={passwordStrength > 2 ? "text-green-500" : "text-muted-foreground"}>{getStrengthText()}</span>
                            </p>
                        </div>

                        {/* Terms Checkbox - Controls Button State */}
                        <div className="flex items-start gap-3">
                            <div className="flex items-center h-5">
                                <input
                                    ...
                                    className="w-4 h-4 rounded border-input bg-input/10 text-primary focus:ring-primary accent-primary cursor-pointer"
                                />
                            </div>
                            <label htmlFor="terms" className="text-xs text-muted-foreground leading-relaxed cursor-pointer select-none">
                                I agree to the <Link href="/terms" className="text-foreground hover:text-primary font-bold transition-colors">Terms of Service</Link> and <Link href="/privacy-policy" className="text-foreground hover:text-primary font-bold transition-colors">Privacy Policy</Link>.
                            </label>
                        </div>

                        {/* Submit Button - Disabled if !agreed or pending */}
                        <button
                            type="submit"
                            disabled={!agreed || isPending}
                            className="w-full bg-primary cursor-pointer hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed text-primary-foreground font-bold py-3.5 rounded-lg transition-all shadow-lg shadow-primary/20 flex items-center justify-center gap-2"
                        >
                            {isPending ? <><Loader2 size={18} className="animate-spin" /> Creating Account...</> : <>Create Account <ArrowRight size={18} /></>}
                        </button>
                    </form>
            ...
            {/* Right Side Visuals (Same as before) */}
            <div className="hidden lg:block w-1/2 bg-secondary relative overflow-hidden">
                ...
                <div className="absolute inset-0 flex items-center justify-center z-10">
                    <div className="relative w-[500px] h-[500px]">
                        <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.5, 0.3] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-primary rounded-full blur-[150px]" />
                        <motion.div animate={{ y: [0, -20, 0], rotate: [-12, -10, -12] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }} className="absolute top-0 left-10 w-64 h-80 bg-background/40 backdrop-blur-xl border border-white/20 rounded-2xl flex flex-col p-6 shadow-2xl">
                        ...
                        <motion.div animate={{ y: [0, 20, 0], rotate: [12, 14, 12] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", delay: 1 }} className="absolute bottom-10 right-10 w-64 h-64 bg-background/40 backdrop-blur-xl border border-white/20 rounded-2xl flex items-center justify-center shadow-2xl">
                ...
                <div className="absolute bottom-12 left-12 right-12 z-20">
                    <blockquote className="text-xl font-heading font-bold text-foreground leading-relaxed mb-6">
                        "Themes Jet provides the cleanest, most reliable templates I've ever used. A total game changer for my workflow."
                    </blockquote>
